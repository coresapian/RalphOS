...
def is_corrupted_html(filepath: Path) -> Tuple[bool, str]:
 """
 Check if an HTML file is corrupted (binary/compressed data).
 ...
 # Check for gzip magic bytes (not decompressed)
 if raw_bytes[:2] == bytes([0x1f, 0x8b]):
 return True, "gzip_compressed"
 ...
 if ratio > 0.3:
 return True, "binary_wrapped"
 return True, "missing_doctype"
...
def validate_stage_2(source_dir: Path) -> ValidationResult:
 ...
 - Files are non-empty
 - Files contain valid HTML (not binary/compressed data)
 ...
 # Validate HTML files with corruption detection
 empty_files = 0
 corrupted_files = 0
 valid_files = 0
 total_size = 0
 ...
 is_corrupted, reason = is_corrupted_html(html_file)
 ...
 corrupted_files += 1
 corruption_reasons[reason] = corruption_reasons.get(reason, 0) + 1
 if corrupted_files <= 5:
 errors.append(f"Corrupted ({reason}): {html_file.name}")
 else:
 valid_files += 1

 stats["empty_files"] = empty_files
 stats["corrupted_files"] = corrupted_files
 stats["valid_files"] = valid_files
 stats["total_size_mb"] = round(total_size / (1024 * 1024), 2)
 stats["avg_size_kb"] = round(total_size / len(html_files) / 1024, 1) if html_files else 0

 # Report corruption breakdown
 if corruption_reasons:
 stats["corruption_reasons"] = corruption_reasons
 for reason, count in corruption_reasons.items():
 warnings.append(f"{count} files corrupted ({reason})")

 # Critical error if more than 10% corrupted
 if corrupted_files > len(html_files) * 0.1:
 errors.append(f"{corrupted_files} corrupted HTML files detected - run: python scripts/tools/html_validator.py {source_dir} --delete")
