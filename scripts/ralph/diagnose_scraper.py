...
Usage:
    python scripts/ralph/diagnose_scraper.py scraped_builds/modified_rides/
...
def diagnose(output_dir: Path) -> int:
    ...
    if failed_count > 0:
        ...
        if blocked > failed_count * 0.5:
            ...
            print(f"    Use: python scripts/ralph/stealth_scraper.py --source {output_dir.name}")
            critical = True
...
def main():
    if len(sys.argv) < 2:
        ...
        print(f"Example: {sys.argv[0]} scraped_builds/modified_rides/")
        sys.exit(1)
