...
calculate_status() {
    ...
    # Pending: no URLs discovered yet
...
show_source() {
    ...
    echo ""
    echo -e "  URLs Expected:   ${expected:-N/A}"
...
show_summary() {
    ...
    echo -e "  Total:         $total"
    echo ""
    echo -e "  Total URLs Found:   $total_urls"
    ...
    if [[ $mismatched -gt 0 ]]; then
        echo ""
        echo -e "  ${RED}⚠ $mismatched sources have incorrect status!${NC}"
...
fix_statuses() {
    ...
    if [[ $fixed -gt 0 ]]; then
        ...
        echo ""
        echo -e "${GREEN}Fixed $fixed source statuses.${NC}"
    else
